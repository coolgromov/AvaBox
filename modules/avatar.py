...
class Avatar(Module):
    ...
    async def appearance(self, msg, client):
        ...
        if subcommand == "rnn":
            ...
            if not user_data["role"]:
                if user_data["gld"] < 50:
                    return
                await self.server.redis.decrby(f"uid:{client.uid}:gld", 50)
            await self.server.redis.lset(f"uid:{client.uid}:appearance", 0, name)
            ...
            if not current_apprnc:
                ...
                await self.server.redis.set(f"uid:{client.uid}:wearing", "casual")
                if apprnc["g"] == 1:
                ...
                for item in const.room_items:
                    await self.server.modules["frn"].add_item(item, "livingroom", client.uid)
                    for i in range(1, 6):
                        await self.server.modules["frn"].add_item(item, f"room{i}", client.uid)
            else:
    ...
    async def clothes(self, msg, client):
        ...
        try:
            if subcommand == "wear":
                ...
                await self.buy_clothes(msg[1], msg[2]["clths"], msg[2]["ctp"], client)
            elif subcommand == "bst":
                await self.buy_clothes_suit(msg[2]["tpid"], msg[2]["ctp"], client)
            else:
    ...
    async def wear_cloth(self, msg, client):
        ...
        await client.send(["a.clths.wear", {"inv": inv, "clths": clths, "ccltn": ccltn, "cn": "", "ctp": ctp}])
    ...
    async def buy_clothes(self, command, clothes, ctp, client):
        ...
        for item in clothes:
            ...
            if name in items or cloth in items:
                continue
            if cloth not in self.clothes_list[gender]:
                return
            (gold, silver, rating, vipOnly, canBuy) = (
                self.clothes_list[gender][cloth]["gold"],
                self.clothes_list[gender][cloth]["silver"],
                self.clothes_list[gender][cloth]["rating"],
                self.clothes_list[gender][cloth]["vipOnly"],
                self.clothes_list[gender][cloth]["canBuy"])
            if not canBuy: return
            if vipOnly:
                if not user_data["premium"]:
                    return
            if clid: to_buy.append(name)
            else: to_buy.append(cloth)
        if not to_buy or user_data["gld"] < gold or user_data["slvr"] < silver:
            return
        pipe = self.server.redis.pipeline()
        pipe.decrby(f"uid:{client.uid}:gld", gold)
        pipe.decrby(f"uid:{client.uid}:slvr", silver)
        pipe.incrby(f"uid:{client.uid}:crt", rating)
        await pipe.execute()
        await self.change_ctp(client.uid, ctp)
        for cloth in to_buy:
    ...
    async def buy_clothes_suit(self, tpid, ctp, client):
        ...
        await self.server.redis.incrby(f"uid:{client.uid}:crt", rating)
        if ctp not in ["casual", "club", "official", "swimwear", "underdress"]:
            return
        await self.change_ctp(client.uid, ctp)
        for cloth in to_buy:
    ...
    async def update_crt(self, uid):
        ...
        for cloth in clothes:
            for tpid in self.clothes_list[gender]:
                if tpid == cloth:
                    crt += self.clothes_list[gender][cloth]["rating"]
                    break
