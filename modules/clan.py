...
class Clan(Module):
    ...
    def __init__(self, server):
        ...
        self.commands = {"grci": self.get_random_clan_ids,
                         ...
                         "dnt": self.donate_clan} #клан коины функция самая нижняя

    async def get_random_clan_ids(self, msg, client): #нет
    ...
    async def create(self, msg, client): #нет
        r = self.server.redis
    ...
    async def dismiss(self, msg, client): #нет
        r = self.server.redis
    ...
    async def load_clans(self, msg, client): #тут clan["coin"]
        ...
        for cid in msg[2]["cids"]:
            ...
            tmp = {"cinv": inv, "st": 0, "mmbs": members, "lvl": clan["lvl"],
                   "crid": clan["owner"], "ttl": clan["name"], "ccn": clan["coin"],
    ...
    async def load_clan_chat_message(self, msg, client): #нет
        # TODO последние сообщения в клане
        return

    async def get_clan(self, cid): #нет
        r = self.server.redis
        pipe = r.pipeline()
        ...
        coin = await r.incrby(f"clans:{cid}:coin", 0)
        return {"name": result[0], "tag": result[1], "icon": result[2],
                ...
                "lvl": int(result[7]), "coin": coin}

    async def join_clan(self, cid, uid): #тут clan["coin"]
        rid = int(uid)
        r = self.server.redis
    ...
    async def leave_clan(self, cid, uid): #тут clan["coin"]
        r = self.server.redis
    ...
    async def donate_clan(self, msg, client): #сама функция доната клан коинов
        clan_id = await self.server.redis.get(f"uid:{client.uid}:clan")
        cid = clan_id
        if not clan_id:
            return
        outcome_gold = int(msg[2]["prc"]["gld"])
        clan_coin = outcome_gold * 3
        await self.server.redis.incrby(f"clans:{cid}:coin", clan_coin)
        await self.server.redis.decrby(f"uid:{client.uid}:gld", outcome_gold)
        await client.send(["cln.dnt", {}])
        members = (await self.get_clan(cid))["members"]
        inv = {"c": {"cfrn": {"id": "cfrn", "it": []},
                     "crc": {"id": "crc", "it": []}}}
        clan = await self.get_clan(cid)
