...
class House(Location):
        ...
        if subcommand == "info":
            ...
            if user_data["role"] >= 3 and not owner_data["role"]:
                if tmid in self.server.online:
                    await self.server.online[tmid].send(["h.r.kcr", {}])
                    return
    ...
    async def _perform_login(self, client):
        await self.server.modules["cm"].send_campaigns(client)
        await client.send(["cp.cht.gbl", {"blcklst": {"uids": []}}])
        await client.send(["nws.hasnews", {"gnexst": True, "gnunr": True}])
        await self.server.modules["sv20"].get_info(client.uid, client)
