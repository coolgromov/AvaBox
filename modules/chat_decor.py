...
class ChatDecor(Module):
    ...
    async def save_chat_decor_model(self, msg, client):
        ...
        if msg[2]["chtnwbt"]:  # new newBubble
            bubble = msg[2]["chtdc"]["bt"]
            if not bubble:
                await r.delete(f"uid:{client.uid}:newBubble")
            elif not await self.server.inv[client.uid].get_item(bubble):
                if not await self.buy_decor(bubble, client):
                    return
            if bubble:
                await r.set(f"uid:{client.uid}:newBubble", bubble)
        ...
        newBubble = await r.get(f"uid:{client.uid}:newBubble")
        text_color = await r.get(f"uid:{client.uid}:tcl")
        spks = ["bushStickerPack", "froggyStickerPack", "doveStickerPack", "jackStickerPack", "catStickerPack", "sharkStickerPack", "korgiStickerPack"]
    ...
    async def buy_decor(self, item, client):
        ...
        if user_data["gld"] < price:
            return False
        r = self.server.redis
        await r.decrby(f"uid:{client.uid}:gld", price)
        await self.server.inv[client.uid].add_item(item, "gm")
        await client.send(["ntf.inv", {"it": {"c": 1, "iid": "", "tid": item}}])
